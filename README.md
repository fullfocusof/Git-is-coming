...
```sh
git add .
git commit -m "Добавлено описание для первых четырех команд"
```
**Задание 4**. Дополните файл style.css. Индексируйте измененные файлы и выполните коммит.
```sh
git add .
git commit -m "Переопределен стиль файла index.html"
```
...
```sh
git remote add origin https://github.com/fullfocusof/LR1
